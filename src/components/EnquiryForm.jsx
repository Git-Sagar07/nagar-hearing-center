...
import { todayISODate, isClinicClosed } from '../services/slotService'
...
export default function EnquiryForm() {
  ...
  function validateStep(current) {
    ...
    if (current === 4) {
      ...
      if (!form.preferredDate) next.preferredDate = 'Please choose a date'
      else if (isClinicClosed(form.preferredDate)) next.preferredDate = "We're closed on Sundays"
      if (!form.preferredTime) next.preferredTime = 'Please choose a time slot'
    }
    setErrors(next)
        ...
        <div className="max-w-2xl mx-auto px-5 md:px-8 text-center">
          <FadeIn>
            ...
            <p className="mt-3 text-navy-700/75">
              Our staff will review and confirm your {form.preferredDate && form.preferredTime ? 'requested slot' : 'request'} shortly
              {form.preferredContact === 'Phone Call' ? ' with a phone call.' : ' on WhatsApp.'}
            </p>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Preferred Date" required error={errors.preferredDate}>
          <input
            ...
            onChange={(e) => {
              update('preferredDate')(e)
              update('preferredTime')('') // a new date invalidates the previously chosen slot
            }}
        ...
        <div className="sm:col-span-2 -mt-2">
          ...
          {errors.preferredTime && (
            <span className="mt-1.5 block text-xs font-semibold text-red-600">{errors.preferredTime}</span>
          )}
        </div>
